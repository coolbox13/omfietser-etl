#!/usr/bin/env python3
"""
Kruidvat Scraper API Service
============================
FastAPI wrapper for the optimized Kruidvat scraper.
...
async def lifespan(app: FastAPI):
    # Startup
    logger.info("🚀 Starting Kruidvat Scraper API Service...")
    ...
    update_status('kruidvat', ScraperStatus.STARTING, "API service initializing...")
    logger.info("✅ Kruidvat Scraper API Service started successfully")
    yield
...
# Create FastAPI app
app = FastAPI(
    title="Kruidvat Scraper API",
    description="Ultra-optimized Kruidvat scraper with job management and progress monitoring",
    version="2.0.0",
    docs_url="/docs",
    redoc_url="/redoc",
    lifespan=lifespan
...
async def root():
    ...
    return {
        "name": "Kruidvat Scraper API",
        "version": "2.0.0",
        "description": "Production-ready API for Kruidvat supermarket scraping with N8N integration",
        "docs_url": "/docs",
        "health_url": "/health",
        "endpoints": {
...
async def get_progress_summary():
    ...
    try:
        ...
        return {
            ...
            "message": f"Plus scraping in progress: {current_progress.get('products_scraped', 0)} products scraped"
        }
...
async def start_scraping(config: ScrapeConfig, background_tasks: BackgroundTasks):
    """Start a new Plus scraping job"""
    
    # Generate unique job ID
    job_id = f"kruidvat_scrape_{uuid.uuid4().hex[:8]}_{int(time.time())}"
...
async def run_scraper_subprocess(job_id: str, config_file: str):
    ...
    try:
        ...
        cmd = [sys.executable, "/app/kruidvat_scraper.py", "--config", config_file]
