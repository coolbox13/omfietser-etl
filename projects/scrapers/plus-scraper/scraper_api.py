#!/usr/bin/env python3
"""
Plus Scraper API Service
========================
FastAPI wrapper for the optimized Plus scraper.
...
async def lifespan(app: FastAPI):
    # Startup
    logger.info("🚀 Starting Plus Scraper API Service...")
    ...
    update_status('plus', ScraperStatus.STARTING, "API service initializing...")
    logger.info("✅ Plus Scraper API Service started successfully")
    yield
...
# Create FastAPI app
app = FastAPI(
    title="Plus Scraper API",
    description="Ultra-optimized Plus scraper with job management and progress monitoring",
    version="2.0.0",
    docs_url="/docs",
    redoc_url="/redoc",
    lifespan=lifespan
...
async def root():
    ...
    return {
        "name": "Plus Scraper API",
        "version": "2.0.0",
        "description": "Production-ready API for Plus supermarket scraping with N8N integration",
        "docs_url": "/docs",
        "health_url": "/health",
        "endpoints": {
...
async def start_scraping(config: ScrapeConfig, background_tasks: BackgroundTasks):
    ...
    job_id = f"plus_scrape_{uuid.uuid4().hex[:8]}_{int(time.time())}"
...
async def run_scraper_subprocess(job_id: str, config_file: str):
    ...
    try:
        ...
        cmd = [sys.executable, "/app/plus_scraper.py", "--config", config_file]
